...
public interface AttendanceRepository extends JpaRepository<AttendanceEntity, Long> {
    ...
    @Query(nativeQuery = true, value = "SELECT DATE_FORMAT(attendance_date, '%Y-%m') AS month, sum(CASE WHEN attendance_status = 'present' THEN 1 ELSE 0 END) " +
            "as present_days, SUM(CASE WHEN attendance_status = 'absent' THEN 1 ELSE 0 END) AS absent_days , COUNT(*) AS total_days FROM attendance " +
            "WHERE attendance_type = 'employee' group by DATE_FORMAT(attendance_date, '%Y-%m') order by  month")
